      ...
      <section className="py-16 border-t border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h3 className="text-3xl font-bold text-white mb-4">All Our Clients</h3>
            <p className="text-gray-300 max-w-2xl mx-auto">
              A comprehensive overview of our valued partnerships across various industries
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
            {clients.map((client, index) => (
              <AnimationWrapper key={index} animation="scaleIn" delay={index * 100}>
                <button
                  onClick={() => goToClient(index)}
                  className={`group p-6 bg-white/10 hover:bg-white/20 backdrop-blur-sm rounded-xl border border-white/20 transition-all duration-300 transform hover:-translate-y-2 ${
                    index === currentClient ? 'ring-2 ring-orange-500 bg-orange-500/20' : ''
                  }`}
                >
                  <div className="w-full h-20 bg-white/95 rounded-lg flex items-center justify-center mb-4 p-3">
                    <img
                      src={client.logo}
                      ...
                      onError={(e) => {
                        ...
                        const parent = target.parentElement;
                        if (parent) {
                          parent.innerHTML = `<div class="text-gray-600 text-xs font-medium text-center">${client.name}</div>`;
                        }
                      }}
                    />
                  </div>
                  <h4 className="text-white font-semibold text-sm mb-1 group-hover:text-orange-300 transition-colors duration-200">
                    {client.name}
                  </h4>
